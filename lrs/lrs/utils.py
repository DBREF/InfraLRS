...
def normalizeRouteId(route):
    ...
    if type(route) is not str:
        if type(route) is float:  # could be integer, try round
            if int(route) == route:
                route = int(route)
        route = str(route)
    return route.lower()
...
def convertDistanceUnits(distance, qgisUnit, lrsUnit):
    ...
    raise Exception(
        "Conversion from QGIS unit %s to Lrs unit %s not supported"
        % (qgisUnit, lrsUnit)
    )
...
def polylineXYPointXY(polyline, distance):
    ...
    for i in range(len(polyline) - 1):
        p1 = polyline[i]
        p2 = polyline[i + 1]
        li = pointsDistance(p1, p2)

        if length <= distance <= length + li:
            d = distance - length
...
def polylineXYSegmentXY(polylineXY, frm, to, oStart=0.0, oEnd=0.0):
    ...
    for i in range(len(polylineXY) - 1):
        ...
        if len(poly) == 0 and frm <= length + li:
            d = frm - length
...
def checkFields(inputLayer, outputLayer):
    ...
    if missingFields:
        QMessageBox.information(
            None,
            "Information",
            "Could not copy field %s. The type is not probably supported by memory provider, try to change field type."
            % " ".join(missingFields),
        )
